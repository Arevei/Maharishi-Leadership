...
const cards = [
  ...
  { val: "250+", label: "Universities worldwide", pos: "top-1/2 -translate-y-1/2 -left-4" },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          ...
          >
            ...
            {cards.map((c, i) => (
              <motion.div
                ...
                className={`absolute ${c.pos} bg-background border border-border/70 px-5 py-4 rounded-md shadow-[0_8px_32px_rgba(0,0,0,0.06)] min-w-[160px] z-20`}
                style={{
                  animation: `float${(i % 2) + 1} ${5 + i}s ease-in-out ${i}s infinite`,
                }}
              >
                <p className="font-serif text-2xl text-primary font-light leading-none">
                  {c.val}
                </p>
                <p className="text-[11px] text-primary/60 mt-1.5">{c.label}</p>
              </motion.div>
            ))}
          ...
          >
            <p className="flex items-center gap-3 text-[hsl(var(--peach-deep))] uppercase text-[11px] tracking-[0.3em] font-medium mb-6">
              <span className="w-7 h-px bg-[hsl(var(--peach-deep))]" /> The Practice
            </p>
            <h2 className="font-serif text-3xl sm:text-4xl md:text-6xl text-primary leading-[1.05] font-light mb-8">
              One practice.{" "}
              <em className="italic text-[hsl(var(--peach-deep))]">Every benefit.</em> For life.
            </h2>
            <div className="space-y-6 text-base sm:text-lg text-primary/70 leading-[1.85] font-light max-w-2xl">
              <p>
                ...
                <strong className="text-primary font-medium">simple, natural technique</strong>{" "}
                practised for twenty minutes, twice a day, sitting comfortably
                with the eyes closed. It is taught one-to-one by a certified
                teacher, and requires no belief, no concentration, and no effort
                to control the mind.
              </p>
              <p>
                During the practice, the mind settles naturally to a state of{" "}
                <strong className="text-primary font-medium">restful alertness</strong>{" "}
                — a fourth major state of consciousness, physiologically distinct
                from waking, dreaming, and sleep. The internal foundation from
                which every other capability operates becomes verifiably stronger.
              </p>
            </div>
